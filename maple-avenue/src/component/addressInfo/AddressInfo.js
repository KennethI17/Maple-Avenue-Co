import { useState } from "react";
import axios from "axios";
import { useHistory } from "react-router-dom";

function AddressInfo(props) {
  const { item } = props;
  const id = item.id;
  console.log(item);

  const [addAddress, setAddAddress] = useState({
    street: "",
    city: "",
    state: "",
    zip: "",
  });

  const history = useHistory();

  const addAddressHandler = (event) => {
    const name = event.target.name;
    const value = event.target.value;
    const tempAddress = { ...addAddress };
    tempAddress[name] = value;
    setAddAddress(tempAddress);
  };

  const addAddressSubmitHandler = () => {
    axios.post("http://localhost:8080/saveAddress").catch((error) => {});
  };

  const purchaseHandler = () => {
    const email = localStorage.getItem("loggedInUser");

    const params = {
      address: addAddress,
      email: localStorage.getItem("loggedInUser"),
    };
    axios
      .get(`http://localhost:8080/purchaseItem?email=${email}&id=${id}`)
      .then((response) => {
        history.push("/store");
        window.location.reload(false);
      });
  };

  return (
    <div>
      <form className="sign-up-container">
        <div className="alert">
          <h2>Shipping address will be your default address.</h2>
          <h3>I will contact you via email with my Venmo information.</h3>
        </div>
        {/* <label for="inputStreet" className="form-label">
            Steet
          </label>
          <input
            onChange={addAddressHandler}
            name="street"
            value={addAddress.street}
            type="text"
            className="form-control"
            id="inputItemName"
          />
          <label for="inputCity" className="form-label">
            City
          </label>
          <input
            onChange={addAddressHandler}
            name="city"
            value={addAddress.city}
            type="text"
            className="form-control"
            id="inputCity"
          />
          <label for="inputState" className="form-label">
            State
          </label>
          <select
            onChange={addAddressHandler}
            name="state"
            value={addAddress.state}
            id="inputState"
            className="form-select"
          >
            <option selected>Choose...</option>
            <option value="AL">AL</option>
            <option value="AK">AK</option>
            <option value="AR">AR</option>
            <option value="AZ">AZ</option>
            <option value="CA">CA</option>
            <option value="CO">CO</option>
            <option value="CT">CT</option>
            <option value="DC">DC</option>
            <option value="DE">DE</option>
            <option value="FL">FL</option>
            <option value="GA">GA</option>
            <option value="HI">HI</option>
            <option value="IA">IA</option>
            <option value="ID">ID</option>
            <option value="IL">IL</option>
            <option value="IN">IN</option>
            <option value="KS">KS</option>
            <option value="KY">KY</option>
            <option value="LA">LA</option>
            <option value="MA">MA</option>
            <option value="MD">MD</option>
            <option value="ME">ME</option>
            <option value="MI">MI</option>
            <option value="MN">MN</option>
            <option value="MO">MO</option>
            <option value="MS">MS</option>
            <option value="MT">MT</option>
            <option value="NC">NC</option>
            <option value="NE">NE</option>
            <option value="NH">NH</option>
            <option value="NJ">NJ</option>
            <option value="NM">NM</option>
            <option value="NV">NV</option>
            <option value="NY">NY</option>
            <option value="ND">ND</option>
            <option value="OH">OH</option>
            <option value="OK">OK</option>
            <option value="OR">OR</option>
            <option value="PA">PA</option>
            <option value="RI">RI</option>
            <option value="SC">SC</option>
            <option value="SD">SD</option>
            <option value="TN">TN</option>
            <option value="TX">TX</option>
            <option value="UT">UT</option>
            <option value="VT">VT</option>
            <option value="VA">VA</option>
            <option value="WA">WA</option>
            <option value="WI">WI</option> <option value="WV">WV</option>
            <option value="WY">WY</option>
          </select>
          <label for="inputZip" className="form-label">
            Zip Code
          </label>
          <input
            onChange={addAddressHandler}
            name="zip"
            value={addAddress.zip}
            type="text"
            className="form-control"
            id="inputZip"
          /> */}
        <div>
          <div className="closebtn">
            <button
              onClick={purchaseHandler}
              className="btn btn-outline-success"
              type="button"
            >
              Submit Purchase
            </button>
          </div>
        </div>
      </form>
    </div>
  );
}

export default AddressInfo;
